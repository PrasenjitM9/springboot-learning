...
/**
 * 严格的日期转换，改变自己DateUtil，在原来基础上增加了setLenient(false);
 * setLenient
...
public class DateUtils {
	...
	public static int getDaysBetween(java.util.Date paramDate1,
		...
		int i = localCalendar2.get(6) - localCalendar1.get(6);//计算天数
		int j = localCalendar2.get(1);//年
	...
	public static java.util.Date addHours(java.util.Date paramDate, int paramInt)
			throws Exception {
		Calendar localCalendar = Calendar.getInstance();
		localCalendar.setTime(paramDate);
		localCalendar.add(Calendar.HOUR, paramInt);
		return localCalendar.getTime();
	}
...
/*			Calendar calendar = Calendar.getInstance();
			...
			calendar1.set(2019, 0, 15);
			
			Calendar calendar2 = Calendar.getInstance();
			calendar2.set(2019, 1, 16);
			
			int days = DateUtils.getDaysBetween(calendar1.getTime(), calendar2.getTime());
			System.out.println(days);
			
			System.out.println((28+32+30+32)/4);
			
			DateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			Calendar calendar3 = Calendar.getInstance();
			calendar3.set(2019, 1, 16);	
			calendar3.add(Calendar.DAY_OF_MONTH, 32);
