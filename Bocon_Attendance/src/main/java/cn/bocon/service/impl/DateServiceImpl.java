...
import cn.bocon.service.IDateService;
...
public class DateServiceImpl implements IDateService {
    ...
    public String getVocationName(DomNodeList<HtmlElement> htmlElements, String date) throws ParseException{
        ...
        if(rst.equals("")){
            ...
            rst = DateServiceImpl.latestVocationName;
        }else if(pastTimeFlag == true){
            //更新《当前时间，且最近一次的可见的假期名
            DateServiceImpl.latestVocationName = rst;
        }
        return rst;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<ChinaDate> dateList = new DateServiceImpl().getCurrentDateInfo();
        ChinaDate today = new DateServiceImpl().getTodayInfo();
        DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd");
        
        System.out.println("本月详情：");
        for(ChinaDate date: dateList){
            System.out.println(dateFormat.format(date.getSolarDate()) + " " + date.getVacationName() + " " + date.isWorkFlag());
        }
        ...
        System.out.println("已发生的最近一次假期:" + DateServiceImpl.latestVocationName);
    }
